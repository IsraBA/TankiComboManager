// core/randomizer/item_list_scanner.js
...
(function () {
    ...
    window.TankiComboManager.ItemListScanner = {
        ...
        scanAllPurchasedItems(maxOnly, requireQuantity) {
            ...
            for (const item of items) {
                ...
                if (maxOnly) {
                    ...
                    const levelText = levelEl ? levelEl.innerText.trim().toUpperCase() : '';
                    if (!levelText.includes('MAX')) continue;
                }

                // סינון רימונים ללא כמות (amountItemsScroll) — רימון שאין לו כמות לא ניתן לבחור
                if (requireQuantity) {
                    const quantityEl = descDevice.querySelector('.GarageItemComponentStyle-amountItemsScroll');
        ...
        scanAllPurchasedAugments(legendaryOnly) {
            ...
            for (const cell of cells) {
                ...
                if (legendaryOnly) {
                    const isLegendary = !!cell.querySelector(`.${DOM.AUGMENT_CATEGORY_LEGENDARY}`);
