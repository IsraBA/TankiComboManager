...
// תיקון כיווניות (bidi) לצ'אט הקרב: רנדרר הקנבס של המשחק מצייר גליפים
// משמאל לימין לפי סדר המחרוזת, בלי שום מודעות ל-RTL — ולכן עברית/ערבית
// נראות הפוכות ("הרוק המ םולש"). toVisual() ממיר טקסט מסדר לוגי לסדר
// ויזואלי, כך שהציור הנאיבי של המשחק מציג אותו נכון: ריצות RTL מתהפכות
// (כולל שיקוף סוגריים), מילים לועזיות ומספרים נשארים בסדרם, וסדר הריצות
// מתהפך כשכיוון הבסיס הוא RTL.
//
// מגבלות ידועות (ראה CLAUDE.md): הודעה שנשברת לכמה שורות תוצג עם סדר
// שורות הפוך (השבירה קורית בתוך המשחק, אחרי ההמרה שלנו); ערבית מוצגת
// באותיות מבודדות כי באטלס הפונט של המשחק אין עיצוב הקשרי.
