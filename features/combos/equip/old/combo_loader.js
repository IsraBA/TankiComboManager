// features/combos/equip/old/combo_loader.js
...
(function () {
  ...
  window.TankiQoL.ComboLoader = {
    ...
    async equipCombo(combo) {
      ...
      try {
        ...
        if (comboData.turret && !removedItems.turret) {
          await window.TankiQoL.BaseItemEquipper.equipItem(
            comboData.turret,
            "Turrets",
            "Turret",
          );
        ...
        if (comboData.turretAugment && !removedItems.turretAugment) {
          await window.TankiQoL.AugmentEquipper.equipAugment(
            comboData.turretAugment,
            "Turrets",
          );
        }
        ...
        if (comboData.protection) {
          ...
          if (shouldEquipProtections) {
            await window.TankiQoL.ProtectionEquipper.equipProtection(
    ...
    findAugmentInList(augmentName) {
      ...
      if (
        searchName === "STANDARD" ||
        searchName.includes("STANDARD SETTINGS") ||
        searchName.includes("STANDARD")
      ) {
        for (let cell of cells) {
          ...
          if (nameEl) {
            ...
            if (
              cellName.includes("STANDARD") ||
              cellName.includes("SETTINGS")
            ) {
              return cell;
            }
          }
    ...
    isItemPurchased(item) {
      ...
      if (item === null || item === undefined) {
        ...
        const equipText = LanguageManager
          ? LanguageManager.getEquipButtonText()
          : "equip";
    ...
    isAugmentPurchased(augmentCell) {
      ...
      if (
        !nameEl ||
        !nameEl.innerText ||
        nameEl.innerText.trim().length === 0
      ) {
      ...
      if (discountCell) {
        ...
        const hasContent =
          discountCell.children.length > 0 ||
          discountCell.innerText.trim().length > 0;
        if (hasContent) {
