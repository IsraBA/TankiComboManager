// features/combos/view/lobby/lobby_shortcut_handler.js
...
(function () {
  ...
  window.TankiQoL.LobbyShortcutHandler = {
    listenerAdded: false,
    keydownHandler: null,
    ...
    updateListenerState() {
      const shouldListen =
        this.isRelevantScreen() && this.isOnMainLobbyScreen();
    ...
    async openGarageAndNavigateToCombos() {
      ...
      const AutoNavigator = window.TankiQoL?.AutoNavigator;
      if (AutoNavigator) {
      ...
      const garageLoaded = await NavigationHelpers.waitForElement(
        DOM.LOBBY_CONTAINER,
      );
      if (!garageLoaded) {
        console.error(
          "[ComboManager] Timeout waiting for garage lobby to load",
        );
        if (AutoNavigator) {
          AutoNavigator.autoNavigateTriggered = false;
        }
      ...
      const turretsBlock = await NavigationHelpers.waitForElementAndClick(
        DOM.LOBBY_TURRETS_BLOCK,
      );
      if (!turretsBlock) {
      ...
      const menuContainer = await NavigationHelpers.waitForElement(
        DOM.MENU_CONTAINER,
      );
      if (!menuContainer) {
