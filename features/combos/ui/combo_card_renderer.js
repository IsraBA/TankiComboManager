...
(function () {
  ...
  window.TankiQoL.ComboCardRenderer = {
    ...
    createComboCard(combo, index, viewRenderer) {
      ...
      // כפתור עריכה. שני האייקונים מוזרקים וה-CSS מציג את הנכון לפי המצב.
      // מכוילים לאייקון ה-X של המשחק: צורה מלאה, קנבס 24×24 מלא, ואותו עובי
      // רצועה (3√2). ה-V תלול יותר כי ב-45° הוא לא מגיע לגובה מלא ברוחב 24;
      // אל תגדילו אותו ב-scale — זה מנפח גם את העובי.
      const editBtnHTML = `
    ...
    createRowsHTML(data, removedItems = {}) {
      ...
      // "NO PAINT" משתמש בתווית הלבנה של שאר החריצים; ה-CSS ממרכז אותה.
    ...
    // שורת התגים בפינה השמאלית התחתונה. מסננת ריקים.
    ...
    // תג בודד — ניתן להסרה כמו כל פריט אחר
    ...
    bindComboCardEvents(card, combo, viewRenderer) {
      ...
      // הסרת פריטים — רק במצב עריכה. אחרת לא עוצרים את האירוע, שיבעבע
      // לכרטיס ויצייד.
      ...
      // לחיצה על הכרטיס = הצטיידות, למעט: מצב עריכה, השורה העליונה, וגרירה.
      // גרירה מזוהה לפי תזוזת העכבר ולא לפי אירועי drag, כי דפדפנים לא
      // עקביים בשאלה אם click נורה אחרי גרירה.
