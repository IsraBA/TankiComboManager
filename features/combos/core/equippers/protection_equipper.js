// features/combos/core/equippers/protection_equipper.js
  ...
  window.TankiQoL.ProtectionEquipper = {
    async equipProtection(protections, removedProtectionIndices = []) {
      ...
      if (window.TankiQoL.TabNavigator) {
        await window.TankiQoL.TabNavigator.navigateToTab("Protection");
      }
      ...
      const desiredState = this.buildDesiredProtections(
        protections,
        removedProtectionIndices,
      );

      // 3. חישוב הפעולות הנדרשות
      const actions = this.calculateProtectionActions(
        currentState,
        desiredState,
      );
      ...
      while (removeActions.length > 0) {
        ...
        for (let i = removeActions.length - 1; i >= 0; i--) {
          ...
          for (let j = 0; j < currentStateAfterRemoval.length; j++) {
            if (
              this.areProtectionsEqual(
                currentStateAfterRemoval[j],
                removeAction.protection,
              )
            ) {
              stillMounted = true;
              break;
      ...
      for (const equipAction of equipActions) {
        await this.equipProtectionAtSlot(
          equipAction.protection,
    ...
    scanCurrentProtections() {
      ...
      const mountedResists = document.querySelectorAll(
        DOM.PROTECTION_MOUNTED_RESIST,
      );
      ...
      for (let i = 0; i < Math.min(mountedResists.length, 4); i++) {
        ...
        const iconImg = mountedResist.querySelector(
          DOM.PROTECTION_RESISTANCE_ICON,
        );
        ...
        const protectionData = ProtectionScanner.findProtectionByIconUrl(
          iconImg.src,
        );
        if (protectionData) {
          ...
          if (iconFileName) {
            ...
            for (let item of items) {
              const resistanceIcon = item.querySelector(
                DOM.PROTECTION_LIST_RESISTANCE_ICON,
              );
              if (resistanceIcon && resistanceIcon.src) {
                const listIconFileName = ProtectionScanner.extractIconFileName(
                  resistanceIcon.src,
                );
                if (listIconFileName === iconFileName) {
                  const descriptionDevice = item.querySelector(
                    DOM.ITEM_DESCRIPTION_DEVICE,
                  );
                  const nameSpan = descriptionDevice
                    ? descriptionDevice.querySelector("span")
                    : null;
                  if (nameSpan) {
                    const protectionName = nameSpan.innerText
                      .trim()
                      .toUpperCase();
    ...
    calculateProtectionActions(currentState, desiredState) {
      ...
      for (let i = 0; i < desiredState.length; i++) {
        if (
          desiredState[i] &&
          !this.isProtectionInList(desiredState[i], desiredProtections)
        ) {
      ...
      for (let i = 0; i < currentState.length; i++) {
        if (
          currentState[i] &&
          !this.isProtectionInList(currentState[i], currentProtections)
        ) {
      ...
      for (let slotIndex = 0; slotIndex < currentState.length; slotIndex++) {
        ...
        if (
          currentProt &&
          !this.isProtectionInList(currentProt, desiredProtections)
        ) {
          protectionsToRemove.push({
            protection: currentProt,
            slotIndex: slotIndex,
          });
        }
      }
    ...
    areProtectionsEqual(protection1, protection2) {
      ...
      if (ProtectionScanner && protection1.image && protection2.image) {
        ...
        const fileName2 = ProtectionScanner.extractIconFileName(
          protection2.image,
        );
    ...
    async equipProtectionAtSlot(protection, slotIndex) {
      ...
      const descriptionDevice = protectionItem.querySelector(
        DOM.ITEM_DESCRIPTION_DEVICE,
      );
      const nameSpan = descriptionDevice
        ? descriptionDevice.querySelector("span")
        : null;
      const foundName = nameSpan
        ? nameSpan.innerText.trim().toUpperCase()
        : "UNKNOWN";
      ...
      if (!elementToClick) {
        ...
        const itemPreview = protectionItem.querySelector(
          ...
          for (let img of images) {
            ...
            if (
              !img.classList.contains(
                "GarageItemComponentStyle-itemResistanceIcon",
              )
            ) {
              elementToClick = img;
              break;
    ...
    async removeProtectionByData(protection) {
      ...
      const protectionIconFileName = ProtectionScanner.extractIconFileName(
        protection.image,
      );
      ...
      for (let i = 0; i < mountedResists.length; i++) {
        ...
        const mountedIconFileName = ProtectionScanner.extractIconFileName(
          iconImg.src,
        );
        if (mountedIconFileName === protectionIconFileName) {
          ...
          // חיפוש כפתור ההסרה
          const deleteButton = mountedResist.querySelector(
            'img[src*="iconDeleteSelect"]',
          );
          if (deleteButton) {
    ...
    findProtectionInList(protection) {
      ...
      const items = document.querySelectorAll(DOM.ITEM_LIST_CONTAINER);
      const searchName = protection.name
        ? protection.name.toUpperCase().trim()
        : "";
      ...
      if (searchImageUrl) {
        const ProtectionScanner = window.TankiQoL.ProtectionScanner;
        if (ProtectionScanner) {
          const searchIconFileName =
            ProtectionScanner.extractIconFileName(searchImageUrl);
      ...
      if (searchName) {
        for (let item of items) {
          ...
          if (
            cleanItemNameUpper === searchName ||
            itemNameInList.includes(searchName)
          ) {
            return item;
          }
        }
