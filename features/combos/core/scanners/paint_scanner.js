  ...
  window.TankiQoL.PaintScanner = {
    ...
    findPaintImageInList() {
      ...
      for (let item of items) {
        ...
        const hasWhiteBoxShadow =
          boxShadow && boxShadow.includes("255, 255, 255");

        if (hasGreenBackground || hasWhiteBoxShadow) {
          // מצאנו את הצבע המצויד! נחלץ את התמונה שלו
          const previewContainer = item.querySelector(
            DOM.ITEM_PREVIEW_CONTAINER,
          );
          if (previewContainer) {
