// features/combos/core/tab_navigator.js
  ...
  window.TankiQoL.TabNavigator = {
    ...
    getAllTabsInOrder() {
      ...
      const tabs = Array.from(
        menuContainer.querySelectorAll(`.${DOM.TAB_ITEM_CLASS}`),
      );
    ...
    getCurrentTabIndex(tabs) {
      ...
      for (let i = 0; i < tabs.length; i++) {
        ...
        const underline = tabs[i].querySelector(
    ...
    async activateTab(tab) {
      ...
      if (tab === this.comboTab) {
        ...
        if (
          this.comboTab &&
          this.comboTab.classList.contains(DOM.ACTIVE_TAB_CLASS)
        ) {
          MenuInjector.deactivateComboTab(
            this.comboTab,
            menuContainer,
            this.comboTabUnderline,
          );
    ...
    async navigateToTab(tabKey) {
      ...
      const tabName = LanguageManager
        ? LanguageManager.getTabName(tabKey)
        : tabKey;
      ...
      if (targetTab) {
        ...
        if (
          ...
          window.TankiQoL.ViewRenderer.viewElement
        ) {
          const comboView = window.TankiQoL.ViewRenderer.viewElement;
