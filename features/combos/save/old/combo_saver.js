// features/combos/save/old/combo_saver.js

// ⚠️ מסלול ישן — לא מחווט. הוחלף ב-instant_saver.js (שמירה מיידית ממצב
// המשחק). נשמר בכוונה כקוד ייחוס / חילוץ־חירום: הוא עובד על ה-DOM בלבד
// ולכן שורד גם שבירה של הגילוי בעולם MAIN. אין לקרוא לו משום מקום.

// זה הקובץ שעושה את הקסם. הוא עובר כל כרטיסייה, מחכה, בודק מה מצויד, נכנס לאוגמנטים, שומר, וחוזר.
// save/old/combo_saver.js
(function () {
  "use strict";

  const DOM = window.TankiQoL.DOM;
  const Utils = window.TankiQoL.Utils;

  window.TankiQoL.ComboSaver = {
    // פונקציה ראשית שנקראת בלחיצת כפתור
    async saveCurrentCombo() {
      // אובייקט שיחזיק את התוצאות
      const currentCombo = {
        turret: null,
        turretAugment: null,
        hull: null,
        hullAugment: null,
        grenade: null,
        drone: null,
        protection: null,
        // paint: null,  // בוטל - פונקציונליות הצבע הוסרה
      };

      try {
        // 1. סריקת תותח
        if (window.TankiQoL.TabNavigator) {
          await window.TankiQoL.TabNavigator.navigateToTab("Turrets");
        }
        currentCombo.turret =
          window.TankiQoL.BaseItemScanner.scanItem();

        // 2. סריקת אוגמנט תותח
        currentCombo.turretAugment =
          await window.TankiQoL.AugmentScanner.scanAugment();

        // 3. סריקת גוף
        if (window.TankiQoL.TabNavigator) {
          await window.TankiQoL.TabNavigator.navigateToTab("Hulls");
        }
        currentCombo.hull = window.TankiQoL.BaseItemScanner.scanItem();

        // 4. סריקת אוגמנט גוף
        currentCombo.hullAugment =
          await window.TankiQoL.AugmentScanner.scanAugment();

        // 5. סריקת רימון
        if (window.TankiQoL.TabNavigator) {
          await window.TankiQoL.TabNavigator.navigateToTab("Grenades");
        }
        currentCombo.grenade =
          window.TankiQoL.BaseItemScanner.scanItem();

        // 6. סריקת דרון (עם פונקציית ניקוי מיוחדת)
        if (window.TankiQoL.TabNavigator) {
          await window.TankiQoL.TabNavigator.navigateToTab("Drones");
        }
        currentCombo.drone = window.TankiQoL.BaseItemScanner.scanItem(
          window.TankiQoL.BaseItemScanner.cleanDroneName.bind(
            window.TankiQoL.BaseItemScanner,
          ),
        );

        // 7. סריקת הגנה
        if (window.TankiQoL.TabNavigator) {
          await window.TankiQoL.TabNavigator.navigateToTab(
            "Protection",
          );
        }
        currentCombo.protection =
          window.TankiQoL.ProtectionScanner.scanProtection();

        // שמירה ל-localStorage
        this.saveToStorage(currentCombo);

        // חזרה לכרטיסיית COMBOS
        if (window.TankiQoL.MenuInjector) {
          const menuContainer = document.querySelector(DOM.MENU_CONTAINER);
          if (menuContainer) {
            const comboTab = Array.from(menuContainer.children).find(
              (el) => el.dataset.cmeComboTab,
            );
            if (comboTab) {
              const underline = comboTab.querySelector(
                `.${DOM.ACTIVE_UNDERLINE_CLASS}`,
              );
              await window.TankiQoL.MenuInjector.safeActivateComboTab(
                comboTab,
                menuContainer,
                underline,
                150,
              );
            }
          }
        }
      } catch (error) {
        console.error("[ComboManager] Error saving combo:", error);
      }
    },

    saveToStorage(comboData) {
      // שליפת קומבואים קיימים
      chrome.storage.local.get(["savedCombos"], (result) => {
        let combos = result.savedCombos || [];

        // קומבו חדש יקבל order = 0, וכל השאר יעלו ב-1
        combos.forEach((combo) => {
          if (combo.order !== undefined) {
            combo.order += 1;
          }
        });

        // זיהוי השפה הנוכחית
        const LanguageManager = window.TankiQoL.LanguageManager;
        const currentLanguageCode = LanguageManager
          ? LanguageManager.getCurrentLanguageCode()
          : "en";

        const newCombo = {
          id: Date.now(), // מזהה ייחודי
          name: `Combo ${combos.length + 1}`,
          data: comboData,
          date: new Date().toLocaleDateString(),
          order: 0, // קומבו חדש תמיד ראשון
          language: currentLanguageCode, // שמירת השפה שבה נשמר הקומבו
        };

        combos.push(newCombo);

        chrome.storage.local.set({ savedCombos: combos }, () => {
        });
      });
    },
  };
})();
